...
void RC_Receive(void){
				...
				if((sbus_rx_buffer[13]==1&&RC_Ctl.mouse.press_r==0)||((((sbus_rx_buffer[5] >> 4)& 0x000C) >> 2)==2&&RC_Ctl.rc.s2==3))
				{
					RC_Ctl.mouse.isUpedge_r=1;
					RC_Ctl.mouse.isDownedge_r=0;
				}
				else if((sbus_rx_buffer[13]==1&&RC_Ctl.mouse.press_r==1)||((((sbus_rx_buffer[5] >> 4)& 0x000C) >> 2)==2&&RC_Ctl.rc.s2==2))
				{
					RC_Ctl.mouse.isUpedge_r=1;
					RC_Ctl.mouse.isDownedge_r=0;
				}else if((sbus_rx_buffer[13]==0&&RC_Ctl.mouse.press_r==1)||((((sbus_rx_buffer[5] >> 4)& 0x000C) >> 2)==3&&RC_Ctl.rc.s2==2))
				{
					RC_Ctl.mouse.isUpedge_r=1;
...
if(me.isStart==1)
{
	...
	if(RC_Ctl.mouse.isClicked_r==1&&RC_Ctl.velocity.isBMSet==0&&RC_Ctl.velocity.isStepperMoving==0)
		{
			...
			{
				RC_Ctl.velocity.BMPWM=BM_PWM_MAX*0.1;
	...
	if(RC_Ctl.mouse.press_l==1||RC_Ctl.rc.s2==1)
		{
