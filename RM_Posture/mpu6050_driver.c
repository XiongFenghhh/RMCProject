...
#define MPU6050_DEVICE_ADDRESS_SECOND 0xE0
#define MPU6050_ID_SECOND 0x70
...
//    HEAT_Configuration();
    ...
    if(IIC_ReadData(MPU6050_DEVICE_ADDRESS_SECOND,WHO_AM_I ,&temp_data ,1)==0) //确定IIC总线上挂接的是否是MPU6050
    {
        if(temp_data != MPU6050_ID_SECOND)
        {
            printf("error 1A\r\n");
            return 0xff; //校验失败，返回0xff
        }
    ...
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,PWR_MGMT_1,0x01) == 0xff)    //解除休眠状态
    {
        printf("error 1C\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,SMPLRT_DIV,0x07) == 0xff)//cyq：07 更新频率1khz
    {
        printf("error 1D\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,CONFIG,0x00) == 0xff)
    {
        printf("error 1E\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,GYRO_CONFIG,0x08) == 0xff)
    {
        printf("error 1F\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,ACCEL_CONFIG,0x08) == 0xff)
    {
        printf("error 1G\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,INT_PIN_CFG,0x00) == 0xff)
    {
        printf("error 1H\r\n");
        return 0xff;
    }
    if(IIC_WriteData(MPU6050_DEVICE_ADDRESS_SECOND,INT_ENABLE,0x01) == 0xff)
    {
        printf("error 1I\r\n");
        return 0xff;
    }
...
void MPU6050_Gyro_calibration(void)
{
	...
	int a=21000; //at 168MHz 42000 is ok
	for(i=0; i<1000; i++)
	{
		printf("====");
		MPU6050_ReadData();
